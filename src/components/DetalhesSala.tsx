...
export default function RoomDetailsModal({ room, isOpen, onClose }: RoomDetailsModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 bg-opacity-30 flex items-center justify-center z-50">
      <Card className="relative w-full max-w-4xl bg-content-secondary p-6 overflow-hidden">
        <button
          onClick={() => {
            onClose();
            setRange(null);
          ...
          <div className="flex flex-col md:flex-row">
            <div className="flex-1 min-w-0">
              <Image
                ...
                width={800}
                height={450}
                className="w-full h-auto object-cover rounded-tl-2xl rounded-tr-2xl md:rounded-tr-none"
                loading='lazy'
              />
            ...
            <div className="w-full md:w-1/3 p-4">
              <Text className="text-content-primary text-2xl font-semibold">{room.nome}</Text>
              <HStack className="flex flex-wrap gap-2 my-6">
