    ...
    return (
        <VStack className="gap-6 mt-6">
            <HStack className="justify-between items-center">
                ...
                {!editView ? (
                    <Button title="Editar" variant="secondary" className="w-fit" onClick={() => setEditView(true)}/>
                ) : (
                    <HStack className="gap-2">
                        <Button title="Cancelar" variant="primary" className="w-fit bg-error-500" onClick={() => setEditView(false)}/>
                        <Button title="Salvar" variant="primary" className="w-fit" loading={isLoading} onClick={() => {
                            setIsLoading(true);
                            setTimeout(() => {
                                setIsLoading(false);
                                setEditView(false);
