...
export default function RoomCreateModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const handleSalvar = async () => {
        setIsLoading(true);
        try {
            const novaSala = await createSala({ numero, andar, valorHora, capacidade });
            ...
            for (const recursoId of recursosSelecionados) {
                await addRecursoSala({ sala: novaSala.id, recurso: recursoId, quantidade: 1 });
            }
    ...
    return (
        ...
        >
            <VStack className="gap-6 p-4">
                ...
                {selectedTab === "imagens" && (
                    <RoomTabImagens imagens={imagens} setImagens={setImagens} isCreating
                        sala={{ numero, andar, valorHora }}
                        setSala={({ numero, andar, valorHora }: { numero: string; andar: string; valorHora: string }) => {
                            setNumero(numero);
                            setAndar(andar);
                            setValorHora(valorHora);
                        }} />
                ...
                <div className="mt-6 place-self-end">
                    <Button title="Criar sala" onClick={handleSalvar} loading={isLoading} />
                </div>
            </VStack>
        </Modal>
    );
